/*
File: basic-cc-query-named-param.cc 
...
static void
query_type(lcb_INSTANCE* instance, const std::string& bucket_name, 
    const std::string& name, const std::string& type)
{
    Rows result{};

    std::string statement = "SELECT x.* FROM `" + bucket_name + R"(` x WHERE x.type=$type LIMIT 10)";
    ...
    check(lcb_cmdquery_named_param(cmd, name.c_str(), name.size(), type_json.c_str(), type_json.size()), "add positional parameter for QUERY comand");
...
main(int, char**)
{
    ...
    query_type(instance, bucket_name, "type", "hotel");
